...
*/
   	    ...
   	    int buflen)
   {
     ...
     /* Parse request line */
      ...
      if (parse->path == NULL)            // abs_path cannot be empty
         parse->path = root_abs_path;
...
*/
   ...
   size_t ParsedRequest_requestLineLen(struct ParsedRequest *pr)
   {
      ...
      strlen(pr->host) + 1 + 1 + strlen(pr->version) + 2;
      if(pr->port != NULL)
      {
         len += strlen(pr->port)+1;
      }
      if(pr->path != NULL && pr->path != root_abs_path)
      {
         len += strlen(pr->path);
      }
      return len;
      ...
      while(1)
      {
         if (numConnections < MAX_CONNECTIONS)
         {
            ...
            else if (pid == 0)
            {
               ...
               while (strcmp(buf, "/r/n") != 0)
               {
                  bzero(buf, MAX_LINE_SIZE);
                  n = recv(new_s,buf,MAX_LINE_SIZE - 1, 0);
                  if (n < 0) 
                     error("ERROR reading from socket");
                  if (n + pos > currentSize)
                  {
               ...
               if (ParsedRequest_parse(req, request, len) < 0) {
                  send(new_s, "Error 400 invalid request.", 50, 0);
                  exit(1);
               }
               ...
               host.sin_port = htons(80);
            	...
            	/*Active open*/
               if ((hostSock = socket(PF_INET, SOCK_STREAM, 0)) < 0)
               {
               ...
               if(send(hostSock, request, pos, 0) <= 0)
               {
               /*close(s);*/
                  perror("client: send \n");
                  exit(1);
               ...
               {
                  bzero(buf, MAX_LINE_SIZE);
                  n = recv(hostSock,buf,MAX_LINE_SIZE - 1, 0);
                  if (n < 0) 
                     error("ERROR reading from socket");
                  if (n + pos > currentSize)
                  {
