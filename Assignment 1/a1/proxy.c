...
//Send reliably.
   ...
   int main(int argc, char* argv[])
   {
      ...
      char buf[MAX_LINE_SIZE],	//Read buffer.
        their_IP[INET_ADDRSTRLEN];	//Their IP string.
      ...
      //Get the address of the connection.
         ...
         else if (pid == 0) 
         {
            ...
            while (1)
            {
               ...
               if (n < 0) 
               {
                  perror("Error on reading in child.");
                  exit(1);
               }
            ...
            if (req->port == NULL)
         	...
         	ParsedHeader_remove(req, "Keep-Alive");
         	
         	printf("%s\n", req->path);
         	
            int rlen = ParsedRequest_totalLen(req);
            if (ParsedRequest_unparse(req, request, rlen) < 0) 
            {
            ...
            strcat(request, "Connection: close\r\n\r\n");
         	...
         	/*printf("And this is what we're sending: \n%s", request);*/
            ...
            for(p = list; p != NULL; p = p->ai_next) 
            {
               inet_ntop(AF_INET, &(((struct sockaddr_in*) p->ai_addr)->sin_addr), 
                  their_IP, sizeof their_IP); 
            	/*printf("Trying to connect to host at: %s\n", their_IP);*/
