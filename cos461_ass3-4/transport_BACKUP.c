...
//Generate and send a SYN packet onto the socket specified.
   static int send_syn_packet(mysocket_t sd, context_t *ctx)
   {
      ...
      header.th_ack = ctx->initial_sequence_num+1;
      header.th_off = 5;
      header.th_flags = TH_SYN;
      header.th_win = RECEIVER_WINDOW;
   
   	//Send the packet along.
      if (stcp_network_send(sd, &header, sizeof(struct tcphdr), NULL) != -1)
      {
         cout << "SYN packet sent with sequence number: " << header.th_seq<<endl;
         return 1;
      }
   ...
   static uint8_t* receive_syn_packet(mysocket_t sd, context_t *ctx)
   {
      ...
      if (hdr->th_flags == TH_SYN)
      {
         ...
         SENDER_WINDOW = MIN(CONGESTION_WINDOW, hdr->th_win);
         return buf;
      }
      else
      {
   ...
   {
      struct tcphdr header;
      header.th_seq = next_to_send;
      header.th_ack = next_to_recv;
      header.th_off = 5;
   ...
   static int send_ack_packet(mysocket_t sd, context_t *ctx)
   {
      ...
      header.th_ack = header.th_ack + 1;
   
   	//Send it along; return 1 if successful.
      if (stcp_network_send(sd, &header, sizeof(struct tcphdr), NULL) != -1)
      {
         cout << "ACK packet send with ACK number: " << header.th_ack << "." <<endl;
         return 1;
      }
      else 
      {
   ...
   static int handle_net_data(mysocket_t sd, context_t *ctx)
   {
      ...
      if (hdr->th_flags & TH_ACK)
      {
         ...
         +  packetsInFlight[0].len <= hdr->th_ack))
         {
 ...
 */
   ...
   */
      if (is_active)
      {
         ...
         next_to_recv = ((struct tcphdr*)buf)->th_seq;
         ...
         next_to_send = next_to_be_acked = ctx->initial_sequence_num;
         free(buf);
      }
      else
      {
 ...
 */
   static void control_loop(mysocket_t sd, context_t *ctx)
   {
      ...
      while (!ctx->done)
      {
         ...
         if (next_to_send >= (next_to_be_acked + SENDER_WINDOW - 1))
