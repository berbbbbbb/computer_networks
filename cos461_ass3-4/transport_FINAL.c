...
/*
//SYN COOKIE CODE
static uint32_t generate_syn_cookie(mysocket_t sd, struct tcphdr* hdr, uint32_t ts)
{
	 ostringstream ibuf;
	 uint32_t result = 0;
    ...
    unsigned char obuf[20];
	 ...
	 network_context_t net_ctx = ctx->network_state;
	 
	 ibuf << net_ctx.peer_addr.sa_family;
	 ibuf << net_ctx.local_addr.sa_family;
	 ibuf << htons(hdr->th_sport);
	 ibuf << htons(hdr->th_dport);
	 ibuf << htons(cookie_secret);
	 ibuf << ts;
	 
	 unsigned char *a=new unsigned char[ibuf.str().size()+1];
	 a[ibuf.str().size()]=0;
	 memcpy(a,ibuf.str().c_str(),ibuf.str().size());
	 
	 
    SHA1((unsigned char*)a, (ssize_t)strlen((const char*)a), obuf);
      ...
      //Check if the packet is an ack packet and set SENDER_WINDOW if it is.
         if (hdr->th_flags & TH_ACK)
         {
            ...
            //CCC << "ACK packet received." << endl;
         	...
         	//SYN COOKIE CODE
         	uint32_t cookie = ntohl(hdr->th_seq) - 1;
         	
         	uint32_t ts = cookie >> 27;
         	uint32_t seq = (cookie << 24)	>> 24;
         	if (seq == generate_syn_cookie(sd, hdr, ts))
         	{*/
            	SENDER_WINDOW = MIN(CONGESTION_WINDOW, ntohs(hdr->th_win));
            	return buf;
            /*}
   ...
   */
      ...
      {   
         ...
         next_to_recv = ntohl(((struct tcphdr*)buf)->th_seq) + 1;
      	...
      	uint32_t mss = 1;
      	uint32_t ts = (seconds % 32);
      	
      	srandom(time(NULL));
      	cookie_secret = random ();
      	
      	uint32_t seq = generate_syn_cookie(sd, (struct tcphdr*)buf, ts);
      	
      	seq = seq >> 8;
      	mss = mss << 24;
      	ts = ts << 27;
      	
      	ctx->initial_sequence_num = ts | mss | seq;
