      ...
      if (packet.len > 0)
      {
         ...
         && (seq_num + packet.len) <= (next_to_recv + RECEIVER_WINDOW))
         {
            ...
            for (unsigned int i = 0; i < packetsReceived.size(); i++)
            {
               ...
               {
                  cout << "Overlapping data already received." << endl;
                  int offset = (seq_num + packet.len - 
                  							packetsReceived[i].sequence_num);
                  uint8_t* buf = malloc(packetsReceived[i].len - 
                  							offset);
   ...
   static void control_loop(mysocket_t sd, context_t *ctx)
   {
      ...
      while (!ctx->done)
      {
         ...
         if (next_to_send >= (next_to_be_acked 
            ...
            event = stcp_wait_for_event(sd, ANY_EVENT, gettimeofday(2));
