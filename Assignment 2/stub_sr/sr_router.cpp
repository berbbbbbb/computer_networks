...
#define TIMEOUT_VALUE 15
   ...
   static void setSignalHandler(int sig, void (*handler)(int));
	...
	/*The paren't default handler for SIGQUIT signals. When called,
 	 sets an alarm and changes SIGQUIT handler to handleQuit2. */
 ...
 *---------------------------------------------------------------------*/
   ...
   {	
      ...
      interface = (char*)memcpy(malloc(strlen(interface)+1), interface, strlen(interface)+1);
      ...
      while (numPackets > 0)
      {
         ...
         {		
            ...
            fixICMPChecksum(buf, size);	
         		...
         		//Send packet
            sr_send_packet(sr, buf, size, interfaces.front());
            free(buf);
 ...
 *---------------------------------------------------------------------*/
        ...
        char* interface/* lent */)
   {
    /* REQUIRES */
      ...
      int ret; //used as ret value to route..
 ...
 *---------------------------------------------------------------------*/
   										...
   										unsigned int len)
   {
      ...
      fixIPChecksum(packet, len);
   	...
   	//Fill the rest with the IP header + 64B of packet.
      for (int i = 14; i < (int)len && i < size-28; i++)
         buf[i+28] = packet[i];
   	
      for (int i = len; i < size-28; i++)
         buf[i+28] = 0;  	
   ...
   static void cacheFromArp(uint8_t* packet)
   {
      ...
      for (int i = 0; i < currentSize; i++)
      {
         if (EntryArray[i]->gw.s_addr == srcIp)
         {
            ...
            cout << "ADDING TO CACHE: " << arpTimes.size() << " " << arpIndices.size() << endl;
            return;
         }
      }  
   }
 ...
 *---------------------------------------------------------------------*/
   static void handleAlarm(int iSig)
   {
      cout << arpTimes.size() << " " << arpIndices.size() << endl;
