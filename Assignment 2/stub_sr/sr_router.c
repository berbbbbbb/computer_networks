...
//Helper Methods.
	...
	//One of the logfiles for unserviced flows.
	ofstream unservedFlows;
...
//Variables.
   ...
   static queue<int> arpTrials;
	...
	//FLOW TABLE METHODS DECLARED AFTER FLOW STRUCTURE BELOW.
	
	//Remove flows that have timed out.
	static void removeTimedOutFlows();
 ...
 * Scope: Local
 *
	A flow structure used for firewalling.
 *---------------------------------------------------------------------------*/
	   struct flow
   {
      uint8_t srcIp[4];
	...
	//Flow table (has to be declared below structure..)
	static vector<struct flow*> flowTable;
	//Check to see if a flow is in the flow table.  
   static int inFlowTable(struct flow* f);
	//Update the time associated with a flow.
	static void updateFlow(struct flow* f);
	//Print a flow to the unserved flows log.
	static void printFlow(struct flow* f, int);
   ...
   {			
      ...
      while (numPackets > 0)
      {
         ...
         if (arpTrials.front() == 5)
         {
            ...
            uint8_t * buf = generateHostUnreachable(packets.front(), 
										lengths.front());
 ...
 *---------------------------------------------------------------------*/
        ...
        char* interface/* lent */)
   {
      ...
      if (firewall)
      {
			...
			for (int i = 0; i < 4; i++)
			{
				f->srcIp[i] = packet[i+26];
				f->dstIp[i] = packet[i+30];
			}
			f->srcPort[0] = packet[35];
			f->srcPort[1] = packet[36];
			f->dstPort[0] = packet[37];
			f->dstPort[1] = packet[38];
			f->proto = packet[23];
			
			//Create the reverse flow.
			struct flow* f2 = (struct flow*)malloc(sizeof(struct flow));
			for (int i = 0; i < 4; i++)
			{
				f2->srcIp[i] = packet[i+30];
				f2->dstIp[i] = packet[i+26];
			}
			f2->srcPort[0] = packet[37];
			f2->srcPort[1] = packet[38];
			f2->dstPort[0] = packet[35];
			f2->dstPort[1] = packet[36];
			f2->proto = packet[23];
		
			//If the connection is the ingress interface AND
			//the flow has no entry in the flow table, FIREWALL IT.
			if (((strcmp(interface, sr->if_list->name) == 0)) 
			&&   (!inFlowTable(f) && !inFlowTable(f2)))
			{
			//Print to the firewall file.
			printFlow(f, 2);
         ...
         else
         {
            ...
            end->next = (struct packetDenied*)malloc(
												sizeof(struct packetDenied));
            end = end->next;
         ...
         cout << "Dropping incoming packet-- firewalled." <<endl;
         ...
         sr_log_pack("dropped.log", start);
			...
			//Try removing flows that have times out (event-based.)
			removeTimedOutFlows();
			//If we have the max number of flows, drop packet.
			if (flowTable.size() == MAX_FLOWS)
			{
				cout << "Flow table full-- dropping packet."<<endl;
				printFlow(f, 3);
				return;
			}
			
			//If the flows aren't in the table, add them
			if (!inFlowTable(f) || !inFlowTable(f2))
			{
				f->timeInserted = f2->timeInserted = time(NULL);
				flowTable.push_back(f);
				flowTable.push_back(f2);
			}
			//Otherwise, just update their timeout information.
			else
			{
				updateFlow(f);
				updateFlow(f2);
         ...
         { 
            ...
            if (hasMacAddress(ret))
            {
               ...
               while (if_list != NULL)
               {
                  if (strcmp(if_list->name, EntryArray[ret]->interface) 
											== 0)
                     break;
                  if_list = if_list->next;
               }
            ...
            else
            {
               ...
               sr = (sr_instance*)memcpy(malloc(sizeof(*sr)), 
																sr, sizeof(*sr));
               packet = (uint8_t*)memcpy(malloc(len), packet, len);
               interface = (char*)memcpy(malloc(strlen(interface)+1), 
               								  interface, strlen(interface)+1);
 ...
 *---------------------------------------------------------------------------*/
   static void removeTimedOutFlows()
	{
		//Iterate over all flows.
		for (int i = 0; i < (int)flowTable.size(); i++)
			//If the time has expired..
			if (difftime(time(NULL), flowTable[i]->timeInserted) >= FLOW_TIME)
			{
				//Free the flow and remove it.
				free(flowTable[i]);
 ...
 *---------------------------------------------------------------------------*/
   static void updateFlow(struct flow* f)
	{
		//Iterate through the flow table, updating the time of the desired flow.
		for (int i = 0; i < (int)flowTable.size(); i++)
			if (f->compare(flowTable[i]) == 0)
				f->timeInserted = time(NULL);
	}
 ...
 *
 	Print a flow to the unserved flows log.
 *---------------------------------------------------------------------------*/	
	static void printFlow(struct flow* f, int code)
	{
		//Open the file if it hasn't been opened yet.
		if (unservedFlows == NULL)
			unservedFlows.open ("unservedflows.log");
		
		//Hack together the output..
		unservedFlows << "< " <<(int)(f->srcIp[0])<<"."<<(int)(f->srcIp[1])<<"."
						  <<(int)(f->srcIp[2])<<"."<<(int)(f->srcIp[3])
		   << ", " <<(int)(f->dstIp[0])<<"."<<(int)(f->dstIp[1])<<"."
			<<(int)(f->dstIp[2])<<"."<<(int)(f->dstIp[3])
			<< ", "; 
			if (f->proto == 17)
				unservedFlows<<"UDP";
			else
				unservedFlows<<"TCP";
			unservedFlows
			<<", "<<(int)(f->srcPort[0])<<(int)(f->srcPort[1])<<", "
					  <<(int)(f->dstPort[0])<<(int)(f->dstPort[1])<<", "
					  <<code<<" >"<<endl;
	}